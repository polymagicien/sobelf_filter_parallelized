...
void apply_blur_filter_everywhere( int width, int height, pixel *p, int size, int threshold )
{
    /*
        Applies blur filter to the whole image
    */
...
int main( int argc, char ** argv )
{
    ...
    if(rank == 0){
        ...
        // Flatten images
        int *flat_imgs[n_images];
        int *flat_imgs_modified[n_images];
        int n_bytes_msg[n_images];

        for(i = 0; i < n_images; i++){
            // Compute addresses
            n_bytes_msg[i] = sizeof(img_info) + (image->width[i] * image->height[i] * sizeof(pixel));
            flat_imgs[i] = (int *)malloc(n_bytes_msg[i]);
            cast_flat_img(flat_imgs[i], &infos, &pixel_array);

            // Fill datas
            infos->width = image->width[i];
            infos->height = image->height[i];
            infos->order = i;
            memcpy(pixel_array, image->p[i], n_bytes_msg[i] - sizeof(img_info));
        }
        ...
        // Dynamic allocation
        int work_distributed = 0;
        int work_received = 0;
        int free_rank = -1;
        int work_to_rank;
        while(work_received < n_images){
            if(free_rank == -1){
                work_to_rank = work_distributed;
            }

            printf("DISTRIBUTED : %d      RECEIVED : %d\n", work_distributed, work_received);
            if(work_distributed < n_images){
                MPI_Isend(flat_imgs[work_distributed], n_bytes_msg[work_distributed]/sizeof(int), MPI_INT, work_to_rank, 0, MPI_COMM_WORLD, &req);
                ++work_distributed;
            }

            /*
                THE LAST FEW IMGS ARE NOT TREATED

            */

            if(work_distributed >= size || work_distributed == n_images){
                MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
                MPI_Get_count(&status, MPI_INT, &n_int_rcvd);
                flat_imgs_modified[work_received] = (int *)malloc(n_int_rcvd*sizeof(int));
                MPI_Irecv(flat_imgs_modified[work_received], n_int_rcvd, MPI_INT, status.MPI_SOURCE, status.MPI_TAG, MPI_COMM_WORLD, &(req_array[work_received]));
                free_rank = status.MPI_SOURCE;
                ++work_received;
            } 
        }
