...
int main( int argc, char ** argv )
{
    ...
    // }
    // MPI_Bcast(&max_pixels, 1, MPI_INT, 0, MPI_COMM_WORLD);
    // if(rank == 0){
        ...
        // Send all images
        for(i=0; i < n_images; i++){
            MPI_Isend(flat_imgs[i], n_bytes_msg[i]/sizeof(int), MPI_INT, (i%(size - 1)) + 1, 0, MPI_COMM_WORLD, &req);
#ifdef DEBUG
    print_flat_img(flat_imgs[i]);
        ...
        if(is_file_performance == 1){
            ...
            fprintf(fp, "%lf (n = %d, n_images = %d) for %s\n", duration, size, n_images, input_filename) ;
            fclose(fp);
        }
